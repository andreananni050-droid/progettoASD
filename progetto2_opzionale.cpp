#include <bits/stdc++.h>
using namespace std;
//----------------------------------
long long res = 0;
long long mmopt = 0;
vector<bool> vis;
vector<vector<array<long long,2>>> adj;
//----------------------------------
vector<array<long long,3>> edges; //
vector<long long> lin;
vector<long long> siz;
vector<long long> weight;
vector<long long> depth;
vector<long long> roots;
vector<array<long long,21>> parent;
map<long long,long long> C;
map<array<long long,2>,long long> M;
long long ccount = 1;
//lettura() è uguale a quello di progettoversione3.cpp
//ma costruisco anche la lista di adiacenza adj
void lettura(){
    vector<long long> p;
    string filename = "19980101.all-paths";

    ifstream myfile(filename);

    string testa, path, coda;
    string num;
    while(getline(myfile, testa, ' ')) {
        getline(myfile, path, ' ');
        stringstream ss(path);
        p.clear();
        while(getline(ss, num, '|')) {
            p.push_back(stoll(num));
        }
        getline(myfile, coda);
        long long len = p.size();
        for(int i = 0;i < len;i++){
            if(!C[p[i]])C[p[i]] = ccount++;
            p[i] = C[p[i]];
        }
        for(int i = 1;i < len;i++){
            if(p[i-1] > p[i])M[{p[i-1],p[i]}]++;
            else M[{p[i],p[i-1]}]++;
        }
    }
    adj.resize(ccount);
    vis.resize(ccount,0);
    for(auto u:M){
        edges.push_back({u.second,u.first[0],u.first[1]});
//------------------------------------------------------------
        adj[u.first[0]].push_back({u.first[1],u.second});
        adj[u.first[1]].push_back({u.first[0],u.second});
//------------------------------------------------------------
    }
}
long long fin(long long x){
    while(x != lin[x])x = lin[x];
    return x;
}
bool same(long long x, long long y){
    return fin(x) == fin(y);
}
void uni(long long x,long long y){
    x = fin(x);
    y = fin(y);
    if(siz[x] < siz[y])swap(x,y);
    lin[y] = x;
    siz[x]+=siz[y];
}
void kruskal(){
    lin.resize(2*ccount+1);
    siz.resize(2*ccount+1);
    depth.resize(2*ccount+1);
    weight.resize(2*ccount+1);
    parent.resize(2*ccount+1);
    roots.resize(2*ccount+1);
    for(int i = 1;i <= 2*ccount-1;i++){
        lin[i] = i;
        siz[i] = 1;
        roots[i] = i;
        weight[i] = 0;
    }
    sort(edges.begin(),edges.end());
    for(auto u:edges){
        if(!same(u[1],u[2])){
            long long x = fin(u[1]);
            long long y = fin(u[2]);
            parent[roots[x]][0] = ccount;
            parent[roots[y]][0] = ccount;
            uni(x,ccount);
            uni(y,ccount);
            roots[fin(ccount)] = ccount; //ccount è la radice nell'albero costruito contenente x,y per ora
            weight[ccount] = u[0];
            ccount++;
        }
    }
    long long root = ccount-1;
    depth[root] = 0;
    for(int i = root-1;i > 0;i--){
        depth[i] = depth[parent[i][0]]+1;
    }
}
void lca_preprocessing(){
    for(int j = 1;j < 21;j++){
        for(int i = 1;i < ccount;i++){
            parent[i][j] = parent[parent[i][j-1]][j-1];
        }
    }
}
long long lca(long long u, long long v) {
    if(depth[u] < depth[v])swap(u,v);
    for(int j = 20;j >= 0;j--) {
        if(depth[u]-depth[v] >= ((long long)1<<j)) {
            u = parent[u][j];
        }
    }
    if(u == v)return u;
    for(int j = 20;j >= 0;j--) {
        if(parent[u][j] != parent[v][j]) {
            u = parent[u][j];
            v = parent[v][j];
        }
    }
    return parent[u][0];
}
long long query(long long u,long long v){
    long long x = C[u];
    long long y = C[v];
    return weight[lca(x,y)];
}


//------------------------------------------------------
//Queste sono le funzioni della parte 2

void dfs(long long cur,long long target) {
    if(cur == target) {
        res++;
        return;
    }
   // cout << "cur = " << cur << endl;
    vis[cur] = true;
    for(auto q:adj[cur]) {
        if(q[1] <= mmopt && !vis[q[0]]) {
            dfs(q[0],target);
        }
    }
    vis[cur] = false;
}
long long query2(long long u,long long v){
    long long x = C[u];
    long long y = C[v];
    mmopt = query(u,v);
    cout << "mmopt = " << mmopt << endl;
    res = 0;
    dfs(x,y);
    return res;
}
//-------------------------------------------------------
int main(){
    lettura();
    kruskal();
    lca_preprocessing();
    long long num_query;
    cin >> num_query;
    while(num_query--){
        long long a,b;
        cin >> a >> b;
        cout << query2(a,b) << endl;
    }
}
